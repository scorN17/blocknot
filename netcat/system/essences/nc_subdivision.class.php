...
/*scorn*/
	$uri_without_params= $uri;
	$this->scorn_realuri= $uri;

    if($uri=='/contacts/') $this->scorn_cityalias= 'rostov-na-donu';
    else{
    	preg_match("/\/contacts\/(([a-z-]{1,})\/){0,1}$/", $uri_without_params, $matches);
    	$this->scorn_cityalias= $matches[2];
    	$uri_without_params= str_replace($matches[1], '', $uri_without_params);
    }

	// preg_match("/\/(x\/(.*)\/)$/", $uri_without_params, $matches);
	// $this->scorn_params= $matches[2];
	// $uri_without_params= str_replace($matches[1], '', $uri_without_params);
    //
	// preg_match("/\/((w[0-9]{6,7})\/)$/", $uri_without_params, $matches);
	// $this->scorn_itemid= $matches[2];
	// $uri_without_params= str_replace($matches[1], '', $uri_without_params);

	$uri= $uri_without_params;
/*scorn*/
